...
const Rating = ({rating, rateSize}: RatingProps) => {
  //  Güvenli kontrol ve varsayılan değer
  const rate = rating?.rate ?? 0;
  const count = rating?.count ?? 0;

  // Math.max ile negatif olmaması ve NaN hatasını önleme
  const fullstar = Math.max(0, Math.floor(rate));
  const halfstar = rate % 1 >= 0.5;
  const emptystar = Math.max(0, 5 - fullstar - (halfstar ? 1 : 0));

  // console.log('Full Stars:', fullstar);
  // console.log('Half Star:', halfstar);
  // console.log('Empty Stars:', emptystar);
